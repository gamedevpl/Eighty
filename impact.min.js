...
this.isUsingKeyboard=true;window.addEventListener('keydown',this.keydown.bind(this));window.addEventListener('keyup',this.keyup.bind(this));},initAccelerometer:function(){if(this.isUsingAccelerometer){return;}
...
var ev=event.touches?event.touches[0]:event;this.mouse.x=(ev.clientX-pos.left)/scale;this.mouse.y=(ev.clientY-pos.top)/scale;},contextmenu:function(event){if(this.bindings[ig.KEY.MOUSE2]){event.stopPropagation();event.preventDefault();}},keydown:function(event){if(event.target!=null){var tag=event.target.tagName;if(tag=='INPUT'||tag=='TEXTAREA'){return;}}
...
event.stopPropagation();event.preventDefault();}},keyup:function(event){if(event.target!=null){var tag=event.target.tagName;if(tag=='INPUT'||tag=='TEXTAREA'){return;}}
